...
public class KeywordTools {
    ...
    public ArrayList<String>  getResearchFields(String researchFName) {
        int index=0;
        for(int i=0; i<researchFieldName.length; i++){
            if(researchFName.equals(researchFieldName[i])){
              index=i;
              break;
            }
        }
        return researchFields[index];
    }
    ...
    public void retrieveKeywordTools() {

        Connection con = DatabaseConnection.getConnection();

        try {
            String sql = "select * from JELO.KEYWORDS";
